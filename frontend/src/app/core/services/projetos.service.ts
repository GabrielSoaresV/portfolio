  ...
  private projetos: ProjetoDetalhado[] = [
    {
      ...
      detalhes: {
        problema: 'A loja física perdeu 60% do faturamento durante a pandemia e precisava de uma presença digital robusta para sobreviver. A plataforma anterior era lenta, tinha taxa de abandono de carrinho de 85% e não integrava com sistemas de pagamento modernos.',
        solucao: 'Desenvolvi uma plataforma completa do zero com Angular para o frontend, Node.js para a API e PostgreSQL como banco de dados. Implementei checkout otimizado, cache inteligente, busca em tempo real e integração nativa com Stripe para pagamentos.',
        resultado: 'Em 6 meses, a loja online superou o faturamento da loja física em 40%. A taxa de abandono de carrinho caiu para 35% e o tempo médio de carregamento das páginas ficou abaixo de 1.5s.'
      },
      funcionalidades: [
        'Carrinho de compras persistente com sincronização em tempo real',
        'Checkout em 3 passos com validação de endereço via CEP',
        'Painel administrativo completo com dashboard de vendas',
        'Sistema de cupons e promoções dinâmicas',
        'Integração com Stripe, Pix e boleto bancário',
        'Notificações por email e WhatsApp para clientes',
        'Sistema de avaliações e Q&A de produtos',
        'Relatórios de vendas exportáveis em PDF e Excel'
      ],
      tecnologias: [
        { nome: 'Angular 17', descricao: 'Framework frontend com SSR para SEO e performance' },
        { nome: 'Node.js + Express', descricao: 'API RESTful com arquitetura em camadas' },
        { nome: 'PostgreSQL', descricao: 'Banco relacional com índices otimizados para busca' },
        { nome: 'Stripe', descricao: 'Processamento de pagamentos com webhooks' },
        { nome: 'Redis', descricao: 'Cache de sessões e dados frequentes' },
        { nome: 'AWS S3', descricao: 'Armazenamento de imagens e assets' }
      ],
      galeria: [
      ...
      documentacao: {
        arquitetura: 'O projeto segue uma arquitetura em camadas com separação clara entre frontend (Angular), API (Node.js/Express) e banco de dados (PostgreSQL). O frontend utiliza standalone components e signals para gerenciamento de estado. A API implementa o padrão Repository Pattern com injeção de dependências.',
